...
export default function CampusMap() {
  ...
  const connectedProfiles = useMemo(() => {
    ...
    return allProfiles.filter((p) => connectedIds.has(p.id) && p.campus_location);
  }, [profile, connections, allProfiles]);

  useEffect(() => {
    if (!mapRef.current || mapInstance.current) return;

    const map = L.map(mapRef.current).setView([SRM_AP_CENTER.lat, SRM_AP_CENTER.lng], 17);
  ...
  useEffect(() => {
    ...
    connectedProfiles.forEach((p) => {
      ...
      L.marker([loc.lat, loc.lng], { icon: connectedIcon })
        ...
        .bindPopup(`<b>${p.full_name}</b><br/>${p.department || ""}<br/>${p.campus_location}`);
    });
  }, [connectedProfiles]);
    ...
    <Layout>
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          See your connected students on the campus map. Only accepted connections are shown for privacy.
        </p>
        <Card>
        ...
        {connectedProfiles.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="text-base">Connected Students on Map</CardTitle></CardHeader>
            <CardContent>
                ...
                {connectedProfiles.map((p) => (
                  <div key={p.id} className="flex items-center gap-2 rounded-lg border p-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{p.campus_location}</p>
                    </div>
                  </div>
                ))}
              </div>
